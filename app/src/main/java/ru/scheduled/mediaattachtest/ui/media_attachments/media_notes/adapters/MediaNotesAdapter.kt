package ru.scheduled.mediaattachtest.ui.media_attachments.media_notes.adapters
...
import ru.scheduled.mediaattachtest.ui.media_attachments.media_notes.CustomLinearLayoutManager
...
class MediaNotesAdapter(
    ...
    private val mediaList = mutableListOf<CustomRecyclerView.MediaNote>()
    ...
    private val selectedNotes = mutableListOf<CustomRecyclerView.MediaNote>()
    ...
    override fun getItemViewType(position: Int): Int {
        return when (mediaList[position].mediaType) {
            CustomRecyclerView.MediaItemTypes.TYPE_SKETCH -> TYPE_SKETCH
            CustomRecyclerView.MediaItemTypes.TYPE_VOICE -> TYPE_VOICE
            CustomRecyclerView.MediaItemTypes.TYPE_PHOTO -> TYPE_PHOTO
            CustomRecyclerView.MediaItemTypes.TYPE_TEXT -> TYPE_TEXT
        }
    }

    fun getSelectedItems(): List<CustomRecyclerView.MediaNote>{
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, s: Int) {
        ...
        if (isSelecting) {
            ...
            if (selectedNotes.contains(mediaList[position])) {
                ...
                holder.itemView.setBackgroundColor(mContext.resources.getColor(R.color.color_primary_light))
    ...
    fun setData(newData: List<CustomRecyclerView.MediaNote>) {
        val oldList = mutableListOf<CustomRecyclerView.MediaNote>().also{
            it.addAll(mediaList)
        }
        mediaList.clear()
        ...
        for (i in firstVisibleItemPosition until firstVisibleItemPosition + visibleItemsSize) {
            ...
            parentRecyclerView.findViewHolderForAdapterPosition(i)?.itemView?.let {
                when (mediaList[i].mediaType) {
                    ...
                    CustomRecyclerView.MediaItemTypes.TYPE_VOICE -> {
